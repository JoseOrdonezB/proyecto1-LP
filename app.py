...
def _dfa_tabla_to_records(tabla: dict) -> list[dict]:
    """Convierte la tabla de transición del DFA a lista de dicts (sin pandas)."""
    if not tabla:
        return []
...
with tab_run:
    ...
    if "run_tokens" in st.session_state:
        ...
        st.dataframe(token_records, use_container_width=True)
